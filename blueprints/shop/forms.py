...
class MpesaPaymentForm(CheckoutForm):
    phone = StringField('Phone', [Length(min=1, max=150)])
...
class CartForm(ProductForm):
    quantity = IntegerField('Quantity', [NumberRange(min=1, max=10000)])
    submit = SubmitField('Submit')
class ReviewForm(FlaskForm):
    rating = IntegerField('Rating', [NumberRange(min=1, max=5)])
    comment = TextAreaField('Comment', [Length(min=1, max=150)])
    submit = SubmitField('Submit')

class ReportForm(FlaskForm):
    report = TextAreaField('Report', [Length(min=1, max=150)])
    submit = SubmitField('Submit')